...
document.addEventListener('DOMContentLoaded', function() {
    ...
    /* =====================================================
       TAURASI SLIDESHOW
       ===================================================== */
    const casestudySlides = document.querySelectorAll('.casestudy-slide');
    const casestudyIndicators = document.querySelectorAll('.casestudy-indicator');
    let casestudyCurrentSlide = 0;
    let casestudyInterval;
    
    function showCasestudySlide(index) {
        if (casestudySlides.length === 0) return;
        
        casestudySlides.forEach((slide, i) => {
            slide.classList.remove('active');
            if (casestudyIndicators[i]) {
                casestudyIndicators[i].classList.remove('active');
            }
        });
        
        casestudyCurrentSlide = index;
        if (casestudyCurrentSlide >= casestudySlides.length) casestudyCurrentSlide = 0;
        if (casestudyCurrentSlide < 0) casestudyCurrentSlide = casestudySlides.length - 1;
        
        casestudySlides[casestudyCurrentSlide].classList.add('active');
        if (casestudyIndicators[casestudyCurrentSlide]) {
            casestudyIndicators[casestudyCurrentSlide].classList.add('active');
        }
    }
    
    function nextCasestudySlide() {
        showCasestudySlide(casestudyCurrentSlide + 1);
    }
    
    function startCasestudySlideshow() {
        if (casestudySlides.length > 1) {
            casestudyInterval = setInterval(nextCasestudySlide, 4000);
        }
    }
    
    function stopCasestudySlideshow() {
    ...
    // Indicator clicks
    casestudyIndicators.forEach((indicator, index) => {
        indicator.addEventListener('click', () => {
            stopCasestudySlideshow();
            showCasestudySlide(index);
            startCasestudySlideshow();
        });
    });
    ...
    /* =====================================================
       IMAGE CAROUSEL - Auto-scroll + Wheel + Touch/Swipe
       ===================================================== */
    ...
    if (carousel && carouselTrack) {
        ...
        function wrapPosition() {
            ...
            if (carouselPos <= -half) carouselPos += half;
            if (carouselPos > 0) carouselPos -= half;
        }
        ...
        document.addEventListener('mousemove', function(e) {
            if (!isDragging) return;
            var delta = e.clientX - touchLastX;
